 ...
 *****/
    ...
    private void sendStore(RouterInfo router, int responseTime) {
        ...
        _log.debug("YOUNG@StoreJob->sendStore:" + msg);
    ...
    private void sendStore(DatabaseStoreMessage msg, RouterInfo peer, long expiration) {
        // YOUNG
        // 记录自己向外发送的dsm
        JSONObject store_json = new JSONObject();
        store_json.put("log_time", utils.getFormatTime());
        store_json.put("target", peer.getRoutingKey().getData());
        if (msg.getEntry().isLeaseSet()) {
            store_json.put("type", "ls");
            store_json.put("ls", utils.ls2json((LeaseSet) msg.getEntry()));
            ...
            if (getContext().keyRing().get(msg.getKey()) != null) {
                ...
                store_json.put("store_way", "sendStoreThroughExploratory");
            } else if (msg.getEntry().getType() == DatabaseEntry.KEY_TYPE_META_LS2) {
                sendWrappedStoreThroughExploratory(msg, peer, expiration);
                store_json.put("store_way", "sendWrappedStoreThroughExploratory");
            } else {
                sendStoreThroughClient(msg, peer, expiration);
                store_json.put("store_way", "sendStoreThroughClient");
            }
        } else {
            store_json.put("type", "ri");
            store_json.put("ri", utils.ri2json((RouterInfo) msg.getEntry()));
            ...
            if (_connectChecker.canConnect(_connectMask, peer)) {
                ...
                store_json.put("store_way", "sendDirect");
            } else {
                sendStoreThroughExploratory(msg, peer, expiration);
                store_json.put("store_way", "sendStoreThroughExploratory");
            }
        }
        utils.aof(utils.getDataStoreDir() + "local_store.json", store_json.toJSONString());
    }
